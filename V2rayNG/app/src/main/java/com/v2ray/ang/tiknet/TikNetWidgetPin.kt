...
/**
 * Pins TikNet home-screen widgets via the system pin flow (API 26+).
 */
object TikNetWidgetPin {
    enum class Kind { Full, Compact }
     ...
     */
    fun requestPin(context: Context, kind: Kind = Kind.Full): Boolean {
        if (!isSupported(context)) return false
        val appWidgetManager = AppWidgetManager.getInstance(context)
        val provider = when (kind) {
            Kind.Full -> ComponentName(context, WidgetProvider::class.java)
            Kind.Compact -> ComponentName(context, CompactWidgetProvider::class.java)
        }
        ...
        val successPending = PendingIntent.getActivity(
            context,
            REQUEST_PIN_WIDGET + kind.ordinal,
            successIntent,
